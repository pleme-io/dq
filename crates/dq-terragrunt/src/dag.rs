...
impl DependencyGraph {
    ...
    pub fn from_directory(root: &Path) -> Result<Self, Error> {
        ...
        let entries: Vec<(PathBuf, Vec<PathBuf>)> = node_map
            .keys()
    ...
    pub fn to_dot(&self) -> String {
        ...
        for edge in self.graph.edge_indices() {
            if let Some((a, b)) = self.graph.edge_endpoints(edge) {
                dot.push_str(&format!(
                    "    \"{}\" -> \"{}\";\n",
                    self.graph[a].relative_path, self.graph[b].relative_path,
                ));
            }
        }
        dot.push_str("}\n");
    ...
    pub fn transitive_dependencies_of(&self, module_path: &Path) -> Vec<&ModuleNode> {
        let canonical = module_path
            .canonicalize()
            .unwrap_or_else(|_| module_path.to_path_buf());
        let Some(&start) = self.node_map.get(&canonical) else {
            return vec![];
        };
    ...
    pub fn to_value(&self) -> Value {
        ...
        root_map.insert(
            Arc::from("root"),
            Value::string(self.root.to_string_lossy().as_ref()),
        );
        root_map.insert(
            Arc::from("module_count"),
            Value::int(self.module_count() as i64),
        );
        root_map.insert(
            Arc::from("edge_count"),
            Value::int(self.edge_count() as i64),
        );
...
"#,
        ...
        assert!(
            a_pos < b_pos,
            "A (depends on B) should come before B in topo sort"
        );
    }
